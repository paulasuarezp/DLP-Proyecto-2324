...
public abstract class AbstractCodeFunction extends ExceptionThrowerVisitor {
    ...
    protected Object generate(AST node, Object param) {
        return node.accept(specification.getVisitor(MaplCodeSpecification.CodeFunction.GENERATE), param);
    }

    protected Object generate(AST node) {
        return generate(node, null);
    }

    protected <T extends AST> void generate(Stream<T> nodes, Object param) {
        nodes.forEach(node -> generate(node, param));
    }

    protected <T extends AST> void generate(Stream<T> nodes) {
        nodes.forEach(this::generate);
    }

    protected <T extends AST> Object generate(Optional<T> optionalNode, Object param) {
        return optionalNode.map(node -> generate(node, param)).orElse(null);
    }

    protected <T extends AST> Object generate(Optional<T> optionalNode) {
        return optionalNode.map(this::generate).orElse(null);
    }
